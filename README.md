# demandfarmAPI
GOT Backend API's For Demand Farm Assignment Provided By KimbB with PostGres in Backend

# SQL CODE

SELECT * FROM public.gottable;


SELECT DISTINCT ON (characters ->> 'characterName') *
FROM public.gottable
ORDER BY (characters ->> 'characterName'), id ASC;


-- Get all characternames based on GreyJoy housename--
SELECT characters -> 'charactername' As cusatomer_names
FROM public.gottable WHERE characters ->> 'houseName' = 'Greyjoy';

-- Get all Objects based on House Name --
SELECT *,characters -> 'houseName' AS house_names
FROM public.gottable WHERE characters ->> 'houseName' = 'Greyjoy';

-- To get all HouseNames --
SELECT DISTINCT housename
FROM public.gottable
CROSS JOIN LATERAL (
...
WHERE characters -> 'houseName' IS NOT NULL;


-- To get all the characters based on HouseName--
SELECT DISTINCT ON (characters -> 'characterName') characters ->> 'characterName' AS charactername, characters, favorite, id
FROM public.gottable
WHERE 
  characters ->> 'houseName' = 'Lannister'
  OR jsonb_typeof(characters -> 'houseName') = 'array' AND characters -> 'houseName' @> '["Lannister"]';


--- Character Search--
SELECT DISTINCT ON (characters -> 'characterName') characters ->> 'characterName' AS charactername, characters, favorite, id
FROM public.gottable
WHERE characterName ILIKE '%sam%'
ORDER BY characters -> 'characterName', id ASC;

-- Works when characterName is empty and houseName has something + works when both characterName and houseName have values --
SELECT DISTINCT ON (characters -> 'characterName') characters ->> 'characterName' AS charactername, characters, favorite, id
FROM public.gottable
WHERE (characters ->> 'houseName' = 'Lannister'
      OR jsonb_typeof(characters -> 'houseName') = 'array' AND characters -> 'houseName' @> '["Lannister"]')
  AND characterName ILIKE '%lan%';

SELECT DISTINCT ON (characters -> 'characterName') characters ->> 'characterName' AS charactername, characters, favorite, id
FROM public.gottable
WHERE (characters ->> 'houseName' IN ('Stark', 'Lannister')
      OR jsonb_typeof(characters -> 'houseName') = 'array' AND characters -> 'houseName' @> jsonb_build_array('Stark', 'Lannister'))
  AND characterName ILIKE '%lan%';
...
SELECT *
FROM public.gottable
WHERE characters ->> 'characterName' = 'Samwell Tarly'
ORDER BY id ASC
LIMIT 1;


-- To update the favorite charecter name object --
UPDATE public.gottable SET favorite = TRUE WHERE id = 43;

UPDATE public.gottable SET favorite = false WHERE id = 19;

SELECT version();

-- To update the favorite charecter name object --
UPDATE public.gottable SET "charactername" = characters ->> 'characterName';

-- get unique character name--
SELECT DISTINCT characters -> 'charactername' FROM public.gottable ;

--Create id--
update public.gottable f
set id = rn
from (
...
INSERT INTO gottable (characters) 
VALUES ('{
